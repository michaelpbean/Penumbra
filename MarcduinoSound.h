...
public:
    ...
    void playSound(uint8_t bank, uint8_t sound)
    {
        ...
        else if (sound != 0)
        {
            ...
            if (sound > fMaxSounds[bank-1])
                fBankIndexes[bank-1] = fMaxSounds[bank-1];
            else
                fBankIndexes[bank-1] = sound;
        }
        ...
        else
        {
            if (bank <= MP3_BANK_CUTOFF)
            {
                ...
                if ((++fBankIndexes[bank-1]) > fMaxSounds[bank-1])
                    fBankIndexes[bank-1] = 1;
                // we'll play the new indexed sound
                sound = fBankIndexes[bank-1];
            }
            else
            {
    ...
    bool sendHCRCommand(const char* cmd)
    {
        if (fModule != kHCR || cmd == nullptr || cmd[0] == '\0')
            return false;
    ...
    bool playHappy(bool extreme = false)
    {
        return sendHCRCommand(extreme ? "SH1" : "SH0");
    }

    bool playSad(bool extreme = false)
    {
        return sendHCRCommand(extreme ? "SS1" : "SS0");
    }

    bool playAngry(bool extreme = false)
    {
        return sendHCRCommand(extreme ? "SM1" : "SM0");
    }

    bool playScared(bool extreme = false)
    {
        return sendHCRCommand(extreme ? "SC1" : "SC0");
    }

    bool playOverload()
    {
        return sendHCRCommand("SE");
    }

    bool enableMuse()
    {
        return sendHCRCommand("M1");
    }

    bool disableMuse()
    {
        return sendHCRCommand("M0");
    }

    bool toggleMuse()
    {
        return sendHCRCommand("MT");
    }

    bool triggerMuse()
    {
        return sendHCRCommand("MM");
    }

    bool setMuseMinSeconds(uint16_t seconds)
    {
        if (fModule != kHCR)
            return false;
        char buffer[12];
        snprintf(buffer, sizeof(buffer), "<MN%u>", unsigned(seconds));
        sendHCR(buffer);
        return true;
    }

    bool setMuseMaxSeconds(uint16_t seconds)
    {
        if (fModule != kHCR)
            return false;
        char buffer[12];
        snprintf(buffer, sizeof(buffer), "<MX%u>", unsigned(seconds));
        sendHCR(buffer);
        return true;
    }

    bool playWavA(uint16_t file)
    {
        if (fModule != kHCR)
            return false;
        char buffer[16];
        snprintf(buffer, sizeof(buffer), "<CA%04u>", unsigned(file % 10000));
        sendHCR(buffer);
        return true;
    }

    bool playHCRSoundA(HCRSound sound)
    {
        const HCRSoundConfig* config = getHCRSoundConfig(sound);
        return (config != nullptr) ? playWavA(config->number) : false;
    }

    bool playWavB(uint16_t file)
    {
        if (fModule != kHCR)
            return false;
        char buffer[16];
        snprintf(buffer, sizeof(buffer), "<CB%04u>", unsigned(file % 10000));
        sendHCR(buffer);
        return true;
    }

    bool playRandomWavA(uint16_t first, uint16_t last)
    {
        if (fModule != kHCR)
            return false;
        char buffer[24];
        snprintf(buffer, sizeof(buffer), "<CA%04uC%04u>", unsigned(first % 10000), unsigned(last % 10000));
        sendHCR(buffer);
        return true;
    }

    bool playRandomWavB(uint16_t first, uint16_t last)
    {
        if (fModule != kHCR)
            return false;
        char buffer[24];
        snprintf(buffer, sizeof(buffer), "<CB%04uC%04u>", unsigned(first % 10000), unsigned(last % 10000));
        sendHCR(buffer);
        return true;
    }

    bool stopVocalizerImmediate()
    {
        return sendHCRCommand("PSV");
    }

    bool stopVocalizerGraceful()
    {
        return sendHCRCommand("PSG");
    }

    bool stopWavA()
    {
        return sendHCRCommand("PSA");
    }

    bool stopWavB()
    {
        return sendHCRCommand("PSB");
    }

    bool setCanonicalMode(bool enabled)
    {
        return sendHCRCommand(enabled ? "OA1" : "OA0");
    }

    bool setPersonalityOverride(bool enabled)
    {
        return sendHCRCommand(enabled ? "O1" : "O0");
    }

    bool resetEmotions()
    {
        return sendHCRCommand("OR");
    }

    bool setHappyEmotion(uint8_t level)
    {
        if (fModule != kHCR)
            return false;
        char buffer[12];
        snprintf(buffer, sizeof(buffer), "<OH%u>", unsigned(std::min<uint8_t>(level, 100)));
        sendHCR(buffer);
        return true;
    }

    bool setSadEmotion(uint8_t level)
    {
        if (fModule != kHCR)
            return false;
        char buffer[12];
        snprintf(buffer, sizeof(buffer), "<OS%u>", unsigned(std::min<uint8_t>(level, 100)));
        sendHCR(buffer);
        return true;
    }

    bool setMadEmotion(uint8_t level)
    {
        if (fModule != kHCR)
            return false;
        char buffer[12];
        snprintf(buffer, sizeof(buffer), "<OM%u>", unsigned(std::min<uint8_t>(level, 100)));
        sendHCR(buffer);
        return true;
    }

    bool setScaredEmotion(uint8_t level)
    {
        if (fModule != kHCR)
            return false;
        char buffer[12];
        snprintf(buffer, sizeof(buffer), "<OC%u>", unsigned(std::min<uint8_t>(level, 100)));
        sendHCR(buffer);
        return true;
    }

    bool playMemory(uint8_t slot)
    {
        if (fModule != kHCR || slot > 9)
            return false;
        char buffer[12];
        snprintf(buffer, sizeof(buffer), "<RRP%u>", unsigned(slot));
        sendHCR(buffer);
        return true;
    }

    bool saveMemory(uint8_t slot)
    {
        if (fModule != kHCR || slot > 9)
            return false;
        char buffer[12];
        snprintf(buffer, sizeof(buffer), "<RRS%u>", unsigned(slot));
        sendHCR(buffer);
        return true;
    }

    bool setOrganicMemoryPlayback(bool enabled)
    {
        return sendHCRCommand(enabled ? "R1" : "R0");
    }
    ...
    {
        if (fModule == kHCR && !fHCRRandomEnabled)
        {
            sendHCR("<MT>");
            fHCRRandomEnabled = true;
        }
        fRandomEnabled = true;
    ...
    void resumeRandomInSeconds(uint32_t seconds)
    {
        ...
        if (fModule == kHCR && fRandomEnabled && !fHCRRandomEnabled)
        {
            sendHCR("<MT>");
            fHCRRandomEnabled = true;
        }
    ...
    void setVolume(float volume)
    {
        ...
        switch (fModule)
        {
            ...
            {
                char buffer[30];
                int level = int(volume * 100);
                snprintf(buffer, sizeof(buffer), "<PVV%d><PVA%d><PVB%d>", level, level, level);
                sendHCR(buffer);
                break;
            }
        }
